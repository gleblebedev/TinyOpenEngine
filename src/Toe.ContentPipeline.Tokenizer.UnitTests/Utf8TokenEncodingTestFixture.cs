...
namespace Toe.ContentPipeline.Tokenizer.UnitTests
{
    ...
    {
        [Test]
        public void IncompleteUtfSequence_SequenceTerminatedAndNextCharacterParsed()
        {
            var array = new byte[] {0b11110000, 0b10010000, 0b10001101, 0b10000000};
            var str = Encoding.UTF8.GetString(array);
            array[array.Length - 1] = (byte) ';';
            var encoding = new Utf8TokenEncoding();
            var span0 = new Span<byte>(array);
            var output = new char[encoding.EstimateCharCount(span0)];
            var res0Len = encoding.GetString(span0, new Span<char>(output, 0, output.Length));
            Assert.AreEqual(str + ";", new Span<char>(output, 0, res0Len).ToString());
        }
        ...
        public void SlicedSequence(byte b0, byte b1, byte b2, byte b3)
        {
