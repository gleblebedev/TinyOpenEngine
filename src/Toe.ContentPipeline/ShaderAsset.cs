...
namespace Toe.ContentPipeline
{
    ...
    {
        public static readonly IShaderParameter<float> DefaultNormalTextureScale =
            new ShaderParameter<float>(ShaderParameterKey.NormalTextureScale, 1.0f);

        public static readonly IShaderParameter<float> DefaultOcclusionTextureStrength =
            new ShaderParameter<float>(ShaderParameterKey.OcclusionTextureStrength, 1.0f);

        public static readonly IShaderParameter<Vector3> DefaultEmissiveFactor =
            new ShaderParameter<Vector3>(ShaderParameterKey.EmissiveFactor, new Vector3(0.0f, 0.0f, 0.0f));
        ...
        public static readonly IShaderParameter<Vector4> DefaultBaseColorFactor =
            new ShaderParameter<Vector4>(ShaderParameterKey.BaseColorFactor, new Vector4(1.0f, 1.0f, 1.0f, 1.0f));

        public static readonly IShaderParameter<float> DefaultMetallicFactor =
            new ShaderParameter<float>(ShaderParameterKey.MetallicFactor, 1.0f);

        public static readonly IShaderParameter<float> DefaultRoughnessFactor =
            new ShaderParameter<float>(ShaderParameterKey.RoughnessFactor, 1.0f);

        //PBR Specular-Glossiness                
        public static readonly IShaderParameter<Vector4> DefaultDiffuseFactor =
            new ShaderParameter<Vector4>(ShaderParameterKey.DiffuseFactor, new Vector4(1.0f, 1.0f, 1.0f, 1.0f));

        public static readonly IShaderParameter<Vector3> DefaultSpecularFactor =
            new ShaderParameter<Vector3>(ShaderParameterKey.SpecularFactor, new Vector3(1.0f, 1.0f, 1.0f));

        public static readonly IShaderParameter<float> DefaultGlossinessFactor =
            new ShaderParameter<float>(ShaderParameterKey.GlossinessFactor, 1.0f);

        private readonly IDictionary<string, IShaderParameter> _extraParameters =
            new Dictionary<string, IShaderParameter>();
        ...
        public virtual IEnumerable<IShaderParameter> Parameters
        {
            get
            {
                if (NormalTexture.Image != null)
                    yield return NormalTexture.AsShaderParameter(ShaderParameterKey.NormalTexture);
                yield return NormalTextureScale.AsShaderParameter(ShaderParameterKey.NormalTextureScale);
                if (OcclusionTexture.Image != null)
                    yield return OcclusionTexture.AsShaderParameter(ShaderParameterKey.OcclusionTexture);
                yield return OcclusionTextureStrength.AsShaderParameter(ShaderParameterKey.OcclusionTextureStrength);
                if (EmissiveTexture.Image != null)
                    yield return EmissiveTexture.AsShaderParameter(ShaderParameterKey.EmissiveTexture);
